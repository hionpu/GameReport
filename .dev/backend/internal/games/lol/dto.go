...
// Account represents a Riot account
...
// Summoner represents a LoL summoner
...
func (p *ParticipantDTO) GetDamageShare(totalTeamDamage int) float64 {
	if totalTeamDamage == 0 {
		return 0
	}
	return float64(p.TotalDamageDealtToChampions) / float64(totalTeamDamage) * 100
}

func (p *ParticipantDTO) GetGoldEfficiency(gameDurationSeconds int) float64 {
	if gameDurationSeconds == 0 {
		return 0
	}
	return float64(p.GoldEarned) / (float64(gameDurationSeconds) / 60.0)
}

func (p *ParticipantDTO) GetVisionScore() int {
	return p.VisionScore
}

func (m *MatchDTO) GetAverageRank() string {
	// This is a placeholder function. Actual implementation would depend on how ranks are stored.
	// You might need to fetch player ranks from another endpoint or service.
	return "Unranked"
}