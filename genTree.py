...
def generate_tags_file(subpath_filter: str = "") -> bool:
    """Generate tags file using ctags command"""
    try:
        if subpath_filter:
            # Run ctags on specific subpath
            cmd = ["ctags", "-R", f"./{subpath_filter}"]
            print(f"Running: ctags -R ./{subpath_filter}")
        else:
            # Run ctags on current directory
            cmd = ["ctags", "-R", "."]
            print("Running: ctags -R .")
        ...
        if result.returncode == 0:
            ...
            print(f"Error generating tags file: {result.stderr}")
            return False
        ...
        print("Error: ctags command not found. Please install Universal Ctags.")
        print("On Ubuntu/Debian: sudo apt-get install universal-ctags")
        print("On macOS: brew install universal-ctags")
        print("On Windows: Download from https://github.com/universal-ctags/ctags/releases")
        return False

def should_regenerate_tags(tags_file: str, subpath_filter: str = "") -> bool:
    """Check if tags file should be regenerated based on modification times"""
    if not os.path.exists(tags_file):
        return True
    
    tags_mtime = os.path.getmtime(tags_file)
    
    # Check if any source files are newer than tags file
    search_path = f"./{subpath_filter}" if subpath_filter else "."
    
    for root, dirs, files in os.walk(search_path):
        for file in files:
            if file.endswith(('.go', '.py', '.js', '.ts', '.java', '.cpp', '.c', '.h', '.hpp', '.cs')):
                file_path = os.path.join(root, file)
                if os.path.getmtime(file_path) > tags_mtime:
                    return True
...
def main():
    ...
    # Always generate a new tags file
    print("Generating tags file...")
    if not generate_tags_file(subpath_filter):
        print("Failed to generate tags file. Exiting.")
        return
    
    if not os.path.exists(tags_file):
        print(f"Error: Tags file '{tags_file}' still not found after generation attempt.")
        return
    ...
    if subpath_filter:
        sanitized_filter = subpath_filter.replace('/', '-').replace('\\', '-')
        
        # Determine the parent directory of the target path
        target_parent = os.path.dirname(subpath_filter)
        if target_parent:
            # Place output in the parent directory of the target
            output_file = os.path.join(target_parent, f"{sanitized_filter}.tree")
        else:
            # Target is in current directory, place output in current directory
            output_file = f"{sanitized_filter}.tree"
    else:
        current_folder = os.path.basename(os.getcwd())
        output_file = f"{current_folder}.tree"
